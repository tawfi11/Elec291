...
int Write_Flash(int address, char * buff, int len) 
{
	DWORD j;
...
int Read_Flash(int address, char * buff, int len) 
{
	DWORD j;
...
unsigned int get_crc16 (int length)
{
	...
	// Assume it takes the microcontroller about 1 second for every 300k bytes of crc16 calculation
...
void print_help (char * prn)
{
	...
	printf("%s -COM3 -w somefile.wav (write 'somefile.wav' to flash via COM3)\n", prn);
	printf("%s -COM3 -w -I somefile.wav (write 'somefile.wav' to flash via COM3, do not check for valid WAV)\n", prn);
	printf("%s -COM3 -v somefile.wav (compare 'somefile.wav' and flash)\n", prn);
	printf("%s -COM3 -Rotherfile.wav (save content of SPI flash to 'otherfile.wav')\n", prn);
	printf("%s -COM3 -P (play the content of the flash memory)\n", prn);
	printf("%s -COM3 -P0x20000,12540 (play the content of the flash memory starting at address 0x20000 for 12540 bytes)\n", prn);
...
int main(int argc, char **argv)
{
    ...
    BOOL b_write=FALSE, b_index_asm=FALSE, b_index_c=FALSE, b_read=FALSE, b_verify=FALSE, b_play=FALSE, b_test=FALSE, b_check=TRUE;
    int play_start, play_length;
    unsigned int crc;
	
    printf("SPI flash memory programmer using serial protocol. (C) Jesus Calvino-Fraga (2012-2019)\n");
    fflush(stdout);
    ...
    for(j=1; j<argc; j++)
    {
    	...
    	else if((argv[j][0]=='-') && (toupper(argv[j][1])=='C') && (toupper(argv[j][2])=='O') && (toupper(argv[j][3])=='M'))
    	{
    		strcpy(m_Serial, &argv[j][1]);
    	}
