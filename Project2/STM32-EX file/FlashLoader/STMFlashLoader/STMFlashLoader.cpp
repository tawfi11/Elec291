...
void man()
{
	...
	printf("  -ft230   Uses the ftdi library and a ft230 USB/serial adapter to activate/reset STM32F051.\n");              
...
bool Is_Option(char* option)
{
	...
	else if (strcmp(option,"-ft230")==0) return true;
    else return false;
}
...
void write_debug_info(char *msg, int page, DWORD addr, float size, STATE status)
{
	...
	if((SHOW_OK && (status == OK)) || (SHOW_KO && (status == KO))) { printf(d_info); fflush(stdout); }
}
		...
		if(pinmode==1)
		{
			if ((ft_eeprom_x_series.Cbus3!=FT_X_SERIES_CBUS_IOMODE) ||  (ft_eeprom_x_series.Cbus0!=FT_X_SERIES_CBUS_IOMODE))
			{
				...
				if (status == FT_OK)
				{
					...
					printf("Congiguring FT230XS IC.  Please wait"); fflush(stdout);
					...
					if(j==40)
					{
				        printf("\nERROR: Can not open FTDI adapter."); fflush(stdout);
					}    
				}
			}
		}
...
void Activate_Bootloader (void)
{
	...
	// CBUS3 is connected to the reset* pin.  CBUS0 is connected to the boot pin (active high).
	...
	for(c=0; c<3; c++)
	{
		FT_SetBitMode(ft_handle, Seq[c], FT_BITMODE_CBUS_BITBANG);
...
void Reset_STM32F051 (void)
{
	...
	FT_SetBitMode(ft_handle, 0x90, FT_BITMODE_CBUS_BITBANG);
	Sleep(10); // Sleep 10 mili seconds
	FT_SetBitMode(ft_handle, 0x98, FT_BITMODE_CBUS_BITBANG);
	Sleep(10);
...
int main(int argc, char* argv[])
{
	...
	else
	{
		...
		while(arg_index < argc)
		{
			...
			//= JCF: Use the ftdi library to identify the first FT230XS and activate bootloader for the STM32F051 =
			else if (strcmp(argv[arg_index],"-ft230")==0) 
			{
				long portn;
				...
				{	    
					...
					{ 
	    				...
	    				{
							char buff[0x100];
							printf("COM%d <USB Serial Port>\n", portn);  fflush(stdout);
							portname=portn;
							sprintf(buff,"echo COM%d>COMPORT.inc", portname);
    						system(buff);
	    				}
					}
			...
			else if (strcmp(argv[arg_index],"-c")==0)
			{
			   while(arg_index < argc)
			   {
				 ...
				 else if(Is_SubOption(argv[arg_index])) // Get connection settings
				 {
					 ...
					 if (strcmp(argv[arg_index-1],"--pn")==0)
					 {
						 portname = atoi(argv[arg_index]);//port name  (e.g COM1, COM2 ..., default COM1) \n");
					 }
				...
				if(portname==0) // JCF: Search for a com port with the bootloader activated
				{
					int j;
					activated_already=0;
					//for(j=1; j<255; j++)
					printf("Searching COM ports:  ");
					fflush(stdout);
					for(j=255; j>0; j--) // This one finds USB adapters faster 
					{
						TARGET_SetComIntType(0);
						SetCOMSettings(j, BaudRate, DataBits, parity, nbStopBit);
						...
						if (Res == SUCCESS)
						{
							...
							{
								char buff[0x100];
								printf(" bootloader found at port COM%d.\n", j);
								fflush(stdout);
								portname=j;
								activated_already=1;
								sprintf(buff,"echo COM%d>COMPORT.inc", j);
    							system(buff);
					...
					if (j%256==0)
					{
						printf(" bootloader not found at any of the available ports.\n");
						fflush(stdout);
						portname=1;
					}
				}
			...
			else if (strcmp(argv[arg_index],"-i")==0)
			{
			   ...
			   DWORD nChars = GetModuleFileName( NULL, szEXEPath, _MAX_PATH );
			   _splitpath(szEXEPath, Drive, Dir, Fname, Ext);
			   ...
			   if (Verify)
			   {
				   ...
				   fflush(stdout);
					...
					if(Do_Reset==1)
					{
						...
						if(FT_Open(0, &ft_handle) == FT_OK)
						{	    
							printf("Appliying reset to STM32F051... "); fflush(stdout);
							Reset_STM32F051();
							printf("done\n"); fflush(stdout);
							FT_Close(ft_handle); // The port will be re-open as an standard serial port
						}
						...
						{
							printf("Could not access FT230 to apply reset.\n"); fflush(stdout);
						}
					}
			   }
				...
				while(arg_index < argc)
				{
				 ...
				 else if(Is_SubOption(argv[arg_index]))
				 {
					 ...
					 else if (strcmp(argv[arg_index-1],"--ewp")==0) 
					 {
						 ...
						 printf("\nenabling write protection %i sectors : ", sectors[0]); fflush(stdout);
