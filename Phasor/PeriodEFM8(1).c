...
void main (void) 
{
    ...
    while (1)
    {
		...
		while(P1_1!=0) // Wait for the signal to be zero
		{
			if(TF1==1) // Did the 16-bit timer overflow?
			{
				TF1=0;
				overflow_count++;
			}
		...
		TR1=0; // Stop timer 0, the 24-bit number [overflow_count-TH0-TL0] has the period!
		period=(overflow_count*65536.0+TH1*256.0+TL1)*(12.0/SYSCLK);
